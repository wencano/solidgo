...
function EditEntityForm() {
  ...
  const initialEntity: Entity = dataAttr ? JSON.parse(dataAttr) : { id: 0, name: '', fields: [] };

  const [name, setName] = createSignal(initialEntity.name);
  const [fields, setFields] = createSignal<Field[]>(initialEntity.fields);
  ...
  const handleSubmit = async (e: Event) => {
    e.preventDefault();
    const response = await fetch(`/entities/${initialEntity.id}`, {
      method: 'PUT',
